...
One emergency encounter (EMIncidentEncounter/EMPOCEncounter/EMEDEncounter) is
expected to produce multiple Condition instances over time — one per
diagnostic stage — distinguished by category and verificationStatus, and
never overwritten in place: chief-complaint (minimum 1), past-medical-history
(0..*, pre-existing comorbidities captured during intake rather than staged
diagnoses of this presentation), working-diagnosis or differential-diagnosis
(0..*), and discharge-diagnosis (0..*, required before the encounter can be
considered diagnostically closed for claims purposes, though that is an
encounter-closure business rule rather than a FHIR cardinality constraint on
this profile).
"""
...
diagnosis); past-medical-history is commonly patient-reported and therefore
often 'unconfirmed'; working-diagnosis typically 'provisional' or
'differential'; discharge-diagnosis typically 'confirmed'. Never submit a
claim against a 'provisional' or 'differential' instance as if it were
'confirmed' — see invariant em-condition-1.
"""
...
Typically 'active' for the chief-complaint/working/differential/discharge
stages — 'resolved'/'inactive' are rare in an acute encounter but keep the
element populated for consistency with facility-side Condition records that
will later track resolution. For category=past-medical-history, 'active' or
'inactive' both apply depending on the checklist item (e.g. an active chronic
condition versus a past surgical history).
"""
...
diagnosis. For category=past-medical-history, code the comorbidity itself
(e.g. hypertension, diabetes, asthma, HIV, TB — matching the ED intake
checklist). For working-diagnosis/discharge-diagnosis, use an ICD diagnosis
...
chief-complaint or a past-medical-history item), but when present this feeds
billing tier / benefit classification, so the profile must be able to carry
it.
"""
...
For category=past-medical-history, this instead captures when the
pre-existing condition itself began, if known.
"""
...
* recorder ^definition = """
Will differ by stage — e.g. a triage nurse for past-medical-history, an EMT
for working-diagnosis, an ED physician for discharge-diagnosis.
"""
...
* asserter ^definition = """
For category=chief-complaint or past-medical-history this is often the
patient or caller themselves, not a clinician — distinguishes 'patient said
chest pain' (asserter=Patient) from 'EMT suspects ACS' (asserter=Practitioner).
"""
